    ...
    while ((Get-Date) -lt $deadline) {
        ...
        try {
            ...
            if ($null -ne $property) {
                ...
                if ($status -in @('success', 'error')) { break }
            }
        } catch {
...
if ($null -ne $historyItem) {
    ...
    if ($errorEntry.Count -gt 0) { $errorMessage = $errorEntry[0][1].exception_message }
}

$report = [ordered]@{
    experiment = 'MiniMax-H3 4090 SeedVR2 portrait image upscale'
...
$markdown = @(
    '# MiniMax-H3 / SeedVR2 portrait image experiment',
    '',
    "- Status: $status",
    "- Prompt ID: $promptId",
...
if ($after.Count -eq 0) { $markdown += '| (none) |  |  |' }
