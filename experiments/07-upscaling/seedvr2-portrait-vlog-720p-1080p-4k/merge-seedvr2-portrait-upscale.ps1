...
$outputPath = Join-Path $outputDir "seedvr2_portrait_vlog_${Target}.mp4"
$ffmpeg = (Get-Command ffmpeg -ErrorAction Stop).Source
$ffprobe = (Get-Command ffprobe -ErrorAction Stop).Source

$inputs = @()
for ($i = 0; $i -lt $SegmentCount; $i++) {
    $leaf = 'seedvr2_portrait_vlog_seedvr2_{0}_seg{1:d2}_*.mp4' -f $Target, $i
...
$evidence | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $experimentDir "merge_${Target}.json") -Encoding UTF8
