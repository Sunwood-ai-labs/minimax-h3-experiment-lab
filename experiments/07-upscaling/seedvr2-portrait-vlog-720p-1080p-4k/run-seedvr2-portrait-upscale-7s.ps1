...
$workflow = Join-Path $experimentDir "workflows/seedvr2_portrait_vlog_7s_${Target}_segment_api.json"
$runDir = Join-Path $experimentDir 'runs'
$manifestPath = Join-Path $runDir "seedvr2_portrait_vlog_7s_${Target}_segments_manifest.json"
...
for ($i = $StartSegment; $i -lt $SegmentCount; $i++) {
    ...
    $name = ('seedvr2_portrait_vlog_7s_{0}_seg{1:d2}' -f $Target, $i)
    $prefix = 'video/seedvr2_portrait_vlog_7s_seedvr2_{0}_seg{1:d2}' -f $Target, $i
    Write-Host "=== $Target 7s segment $i start=$start duration=$duration ==="
    ...
    if (-not $?) { throw "$Target 7s segment $i failed" }
    $rows += [ordered]@{
        target = $Target
        segment = $i
        start_time = $start
