...
python scripts/build-h3-motion-context-workflows.py `
  --output-dir experiments/2026-08-09/niko-h3-motion-context-3segment/workflows
```

各API workflowはComfyUIの`/prompt`へ、トップレベルに`prompt`を付けて投入する。

```powershell
$workflow = Get-Content -Raw experiments/2026-08-09/niko-h3-motion-context-3segment/workflows/segment_01_api.json | ConvertFrom-Json
