...
- Status: `verified`
- Scope: the existing three-reference R2V experiment, four scenes, approximately seven seconds per scene
- Purpose: measure Sol-Attn, generic FP16 CUDA SageAttention, and EasyCache on the same four-scene ref2va R2V workload, using the unoptimized run as the baseline.
...
> The tracked tile is the public visual evidence. The four optimized MP4s remain local-only capture artifacts.

## X video and reference URLs

- **Generated video post**: [X post](https://x.com/hAru_mAki_ch/status/2086051398735847471) — `uncertain`. This is a post-simulation URL; attachment correspondence with the four optimized MP4s has not been confirmed.
- **Reference sources**: [official R2V workflow](https://github.com/Comfy-Org/workflow_templates/blob/main/templates/video_minimax_h3_r2v.json) / [SolAttn](https://github.com/kijai/ComfyUI-SolAttn_triton) / [KJNodes](https://github.com/kijai/ComfyUI-KJNodes) / [SageAttention](https://github.com/thu-ml/SageAttention) / [comparison X post](https://x.com/sunbaolong_2001/status/2085689404031672372?s=46)
- **Workflow-to-video mapping**:

  | Workflow | Scene | X video |
  |---|---|---|
  | [`scene-01`](./workflows/scene-01-sunset-meeting_api.json) | sunset meeting | [X post](https://x.com/hAru_mAki_ch/status/2086051398735847471) (attachment correspondence unconfirmed) |
  | [`scene-02`](./workflows/scene-02-rainy-stairs_api.json) | rainy stairs | [X post](https://x.com/hAru_mAki_ch/status/2086051398735847471) (attachment correspondence unconfirmed) |
  | [`scene-03`](./workflows/scene-03-glasshouse-orbit_api.json) | glasshouse orbit | [X post](https://x.com/hAru_mAki_ch/status/2086051398735847471) (attachment correspondence unconfirmed) |
  | [`scene-04`](./workflows/scene-04-rooftop-pullback_api.json) | rooftop pull-back | [X post](https://x.com/hAru_mAki_ch/status/2086051398735847471) (attachment correspondence unconfirmed) |

## Conclusion and measured speedup
...
All four scenes completed at `1280×704`, 175 frames, 24 fps, and 20 steps, using the same three reference sheets and the same seeds as the baseline. The measured optimized average was **353.145 seconds**, versus **738.794 seconds** for the baseline: **2.092× faster on average**. The best measured scene speedup was **2.141×**. The advertised 3.2× maximum was not reproduced under these exact ref2va conditions.

All four MP4s are valid H.264 + AAC, 175 frames, 7.292 seconds, and have `blackdetect` interval count 0. No OOM or CUDA failure occurred in the final configuration.
...
Sol-Attn timing includes per-prompt autotune overhead. EasyCache skipped 7 or 8 of the 20 sampling steps depending on the scene.
...
## Four scene results and workflow/run mapping

Baseline values come from [the previous verified four-scene experiment](../../03-reference-conditioned/multi-reference-r2v-4scenes-7s/README.md). Optimized values come from the saved reports.

| Scene | Workflow | Description | Seed | Prompt ID | Baseline | Optimized | Speedup | EasyCache | Media |
|---:|---|---|---:|---|---:|---:|---:|---|---|
| 1 | [`scene-01`](./workflows/scene-01-sunset-meeting_api.json) | Sunset rooftop meeting / dolly-in | `2026080813` | `c89dcb38-d2ad-496b-8409-f204694d7406` | 738.494 sec | 349.394 sec | 2.114× | 7/20 | 1280×704, 175 frames, black 0 |
| 2 | [`scene-02`](./workflows/scene-02-rainy-stairs_api.json) | Rainy exterior stairs / tracking shot | `2026080814` | `d20498fe-66fc-4550-a14a-ca86987f4d2e` | 738.442 sec | 365.705 sec | 2.019× | 7/20 | 1280×704, 175 frames, black 0 |
| 3 | [`scene-03`](./workflows/scene-03-glasshouse-orbit_api.json) | Glasshouse conversation / half-orbit | `2026080815` | `4b4b201a-9502-497b-8b80-defd4c8667a1` | 739.257 sec | 352.304 sec | 2.098× | 8/20 | 1280×704, 175 frames, black 0 |
| 4 | [`scene-04`](./workflows/scene-04-rooftop-pullback_api.json) | Rooftop skyline / pull-back | `2026080816` | `f9903bb3-52aa-442b-82c9-da2d4e5d1c04` | 738.981 sec | 345.178 sec | 2.141× | 8/20 | 1280×704, 175 frames, black 0 |
...
Scene 1's official timing report is the independent diagnostic prompt `c89dcb38...`. The later normalized-filename submission had the identical graph and seed and was served from ComfyUI's node cache in 3.137 seconds; that cache replay is not used as a generation-performance result.
...
The full prompts are embedded in the API workflows. Scene prompt summaries:
...
- The VAE runtime patch logged finite audio/video tensors for all four final outputs.
...
- Sol-Attn is approximate attention; compare outputs with the baseline before claiming no quality loss.
...
## Evidence files and source links
...
- [Source X post: MiniMax-H3 acceleration report with Sol-Attn + SageAttention + EasyCache](https://x.com/sunbaolong_2001/status/2085689404031672372?s=46)

The full Japanese/mixed record is [`README.md`](./README.md), and the structured record is [`experiment.json`](./experiment.json).
