...
$inputDir = Join-Path $projectRoot 'runtime/4090/input/h3_mv'
$videoOutputDir = Join-Path $outputDir 'video/h3_mv'
$progressPath = Join-Path $experimentDir 'run-progress.json'
$ffprobe = (Get-Command ffprobe -ErrorAction SilentlyContinue).Source
$ffmpeg = (Get-Command ffmpeg -ErrorAction SilentlyContinue).Source

if (-not (Test-Path -LiteralPath $WorkflowDir -PathType Container)) { throw "Workflow directory not found: $WorkflowDir" }
if ($null -eq $ffprobe -or $null -eq $ffmpeg) { throw 'ffprobe and ffmpeg are required on PATH.' }
...
function Get-ExecutionTiming {
    ...
    if ($startMessage.Count -gt 0) { $executionStart = [DateTimeOffset]::FromUnixTimeMilliseconds([long]$startMessage[0][1].timestamp).LocalDateTime }
    if ($endMessage.Count -gt 0) { $executionEnd = [DateTimeOffset]::FromUnixTimeMilliseconds([long]$endMessage[0][1].timestamp).LocalDateTime }
    [ordered]@{
    ...
    try {
        ...
        $blackEvents = @(
            & $ffmpeg -hide_banner -loglevel info -i $Path -vf 'blackdetect=d=0.1:pic_th=0.98' -an -f null NUL 2>&1 |
                ForEach-Object { $_.ToString() } | Where-Object { $_ -match 'black_(start|end)' }
        )
...
function Save-Progress {
    ...
    $Manifest | ConvertTo-Json -Depth 40 | Set-Content -LiteralPath $progressPath -Encoding UTF8
}
...
$manifest = if ($StartSegment -gt 1 -and $null -ne $existingManifest) { $existingManifest } else {
    [ordered]@{
        status = 'running'
...
for ($segment = $StartSegment; $segment -le 5; $segment++) {
    ...
    if (-not (Test-Path -LiteralPath $workflowPath -PathType Leaf)) { throw "Workflow not found: $workflowPath" }
    ...
    if ([string]::IsNullOrWhiteSpace($promptId)) { throw "ComfyUI did not return prompt_id for segment $segment." }
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        try {
            ...
            if ($null -ne $property) {
                ...
                if ([string]$historyItem.status.status_str -in @('success', 'error')) { break }
            }
        } catch { }
    }
    ...
    $outputFile = Get-ChildItem -LiteralPath $videoOutputDir -Filter ("segment_{0:00}_*.mp4" -f $segment) -File |
