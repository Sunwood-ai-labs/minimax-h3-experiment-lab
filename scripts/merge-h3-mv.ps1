...
try { $ErrorActionPreference = 'Continue'; $ffmpegOutput = & $ffmpeg @ffmpegArgs 2>&1 | Out-String; $exitCode = $LASTEXITCODE } finally { $ErrorActionPreference = $oldEap }
...
$oldEap = $ErrorActionPreference
try { $ErrorActionPreference = 'Continue'; $blackEvents = @(& $ffmpeg -hide_banner -loglevel info -i $mergedPath -vf 'blackdetect=d=0.1:pic_th=0.98' -an -f null NUL 2>&1 | ForEach-Object { $_.ToString() } | Where-Object { $_ -match 'black_(start|end)' }) } finally { $ErrorActionPreference = $oldEap }
...
if (Test-Path -LiteralPath $progressPath -PathType Leaf) {
    ...
    $progress.completedAt = (Get-Date).ToString('o')
